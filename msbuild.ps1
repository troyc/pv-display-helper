...
if ($Type -eq "free") {
	Run-MSBuild $solutionpath "display-driver-helper.sln" $configuration["free"] $platform[$Arch]
}
elseif ($Type -eq "checked") {
	Run-MSBuild $solutionpath "display-driver-helper.sln" $configuration["checked"] $platform[$Arch]
}
elseif ($Type -eq "sdv") {
	$archivepath = "display-driver-helper"

	if (-Not (Test-Path -Path $archivepath)) {
		New-Item -Name $archivepath -ItemType Directory | Out-Null
	}

	Run-MSBuildSDV $solutionpath "display-driver-helper" $configuration["sdv"] $platform[$Arch]
